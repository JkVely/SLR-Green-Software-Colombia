...
legend entries={T\'ecnico (25),Pol\'itica (8),Gesti\'on (10),Educaci\'on (1)}
]
\addplot coordinates {(Colombia,6) (LAC,11) (Global,4) (China,4)};
\addplot coordinates {(Colombia,1) (LAC,5) (Global,1) (China,1)};
\addplot coordinates {(Colombia,4) (LAC,4) (Global,1) (China,1)};
\addplot coordinates {(Colombia,0) (LAC,1) (Global,0) (China,0)};
\node[above,font=\scriptsize] at (axis cs:Colombia,11.5) {11};
\node[above,font=\scriptsize] at (axis cs:LAC,22.5) {21};
\node[above,font=\scriptsize] at (axis cs:Global,7.5) {6};
\node[above,font=\scriptsize] at (axis cs:China,7.5) {6};
\end{axis}
\end{tikzpicture}
...
La Figura~\ref{fig:tendencia} resume la evoluci\'on temporal del corpus: la producci\'on se concentra en los \'ultimos cinco a\~nos, con un pico de 9 estudios en 2026, mientras que 2010, 2015 y 2018 no registran publicaciones, un vac\'io que se discute en Limitaciones.
...
La composici\'on del corpus sesga hacia lo t\'ecnico (19 de 44 art\'iculos), por lo que las afirmaciones sobre pol\'itica y formaci\'on descansan en un subconjunto minoritario y deben leerse como evidencia exploratoria. La b\'usqueda abarc\'o Scopus, IEEE Xplore, ACM Digital Library y Google Scholar; no obstante, no se identificaron estudios con afiliaci\'on colombiana en ACM en la ventana revisada, lo que refuerza la escasez de producci\'on local. La revisi\'on fue liderada por un autor y careci\'o de validaci\'on inter-rater; no obstante, se aplic\'o una r\'ubrica de evaluaci\'on de calidad de 8 criterios adaptada de Kitchenham, puntuada mec\'anicamento desde los metadatos del frontmatter (criterios C1--C8, ver Ap\'endice C). Esta r\'ubrica permite reproducir la puntuaci\'on de cada paper sin necesidad de re-leer el texto completo, controlando as\'i un sesgo de evaluaci\'on \'unico dentro de los l\'imites de los datos disponibles. La distribuci\'on temporal del corpus presenta a\~nos sin publicaciones (2010, 2015, 2018), lo que refleja la heterogeneidad del campo. Solo se incluyeron art\'iculos en ingl\'es o espa\~nol, excluyendo contribuciones en chino y portugu\'es; la b\'usqueda se cerr\'o en agosto de 2026. El an\'alisis es cualitativo y de s\'intesis, no un meta-an\'alisis cuantificado. Las fuentes clim\'atico-energ\'eticas son contextuales y no forman parte del corpus PRISMA; su selecci\'on fue dirigida y su uso como marco contextual es expl\'icito. Este trabajo se enmarca como mapeo exploratorio inicial y no pretende generalizaciones definitivas del estado del arte mundial.
...
27 & Parcial & Material disponible a solicitud \\
\bottomrule
\end{tabular}%
}
