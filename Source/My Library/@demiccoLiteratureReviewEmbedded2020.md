---
citationKey: demiccoLiteratureReviewEmbedded2020
title: A Literature Review on Embedded Systems
itemType: journalArticle
pais: Argentina
region: LAC
enfoque: technical
creators:
  - L. De Micco
  - F.L. Vargas
  - P.I. Fierens
publication: IEEE Latin America Transactions
date: "2020"
year: 2020
tema_asunto: "Sistemas Embebidos, IoT, Green Computing, Edge Computing, Cloud Computing"
escenario_aplicacion: "industria y academia (América Latina)"
institucion_principal: "Universidad Nacional de Mar del Plata"
ciudad: "Mar del Plata"
metodologia: "Revisión de literatura"
url: https:__www.scopus.com_pages_publications_85084442364?origin=resultslist
doi: 10.1109_TLA.2020.9085271
tags:
  - "#ASIC"
  - "#Agrotechnology"
  - "#Bioengineering"
  - "#Communications"
  - "#Computer_Architecture"
  - "#Cryptography"
  - "#DSP"
  - "#Daily_lives"
  - "#Digital_Signal_Processing"
  - "#Embedded_Software"
  - "#Embedded_software"
  - "#Embedded_systems"
  - "#FPGA"
  - "#Green_computing"
  - "#HDL"
  - "#Industry_4_0"
  - "#Internet_of_Things__IOT_"
  - "#Internet_of_things"
  - "#IoT"
  - "#Latin_America"
  - "#Literature_reviews"
  - "#Personal_computers"
  - "#Processing_time"
  - "#RTOS"
  - "#Robotics"
  - "#Wireless_Sensor_Networks"
zotflow-locked: true
zotero-key: B7C7FAMA
item-version: 383
library-id: 20202832
fuente: SCOPUS
decision: incluido
---
# A Literature Review on Embedded Systems
## Abstract
> Embedded system design is crucial for the development of industry, technology, and science, and it is an area that has significantly grown in recent years throughout Latin America, both in academia and in industry. Embedded System _ES_ refers to electronic equipment with a computing core which, unlike a personal computer, is designed to meet a specific function and is usually optimized to satisfy strict requirements of processing time, reliability, power consumption, size, and cost. With the advancement of research on the domains of Internet of Things _IoT_, Computing Edge _CE_ and Cloud Computing _CC_ and their endless application possibilities, ESs have gained new roles and assumed unquestionable importance in our daily lives. In this article, we make a brief review of the relevant literature in the area of embedded systems in the Latin America region during recent years. © 2003-2012 IEEE.

## Notas clave
- ATENCIÓN PRISMA: Registro duplicado con @demiccoLiteratureReviewEmbedded2019 _mismos autores, título y revista_ DOI distinto_. Conservar esta versión 2020 _18 citas frente a 5_ y excluir la otra en el cribado.
- Revisión de literatura sobre sistemas embebidos en América Latina_ la computación verde aparece como un criterio de diseño _optimización de consumo energético, confiabilidad y costo_, no como objeto central de estudio.
- Evidencia del crecimiento regional de IoT, Edge y Cloud Computing y su impacto en el diseño de hardware embebido en LatAm.
- Uso sugerido: contexto regional LatAm en introducción o antecedentes_ no aporta directamente al estado de políticas ni desarrollos Green Cloud específicos.

## Notes
<!-- ZF_NOTE_BEG_KVV7QI95 -->
Export Date: 25 August 2026_ Cited By: 5_ Correspondence Address: L. De Micco_ Instituto de Investigaciones Cientificas y Tecnologicas en Electronica Facultad de Ingenieria, Univ. Nac. de Mar Del Plata Consejo Nacional de Investigaciones Cientificas y Tecnicas Mar Del Plata, Buenos Aires, Argentina_ email: ldemicco_@fi.mdp.edu.ar

<!-- ZF_NOTE_END_KVV7QI95 -->

<!-- ZF_NOTE_BEG_PKUQEVVM -->
Export Date: 25 August 2026_ Cited By: 18

<!-- ZF_NOTE_END_PKUQEVVM -->

